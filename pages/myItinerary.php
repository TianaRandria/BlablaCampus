...
-->
    <?php
    for ($i = 0; $i < count($_SESSION['id_traject']); $i++) {
        echo '
        <div class="flex w-full justify-between items-center rounded-lg bg-xtraLightGrey p-3 h-24 relative cardTraject" id="' . $_SESSION['id_traject'] . '">
            <div class="flex w-3/5 justify-between items-center h-full">
                <div class="flex flex-col">
                    <p class="jourDate font-bungee text-redOnline text-4xl">' . $_SESSION['date_traject'] . '</p>
                    <p class="moisDate font-bungee text-xl">SEP</p>
                </div>
                <div class="h-full flex flex-col items-start justify-center">
                    <p class="startingPoint text-lightGrey font-medium text-sm font-epilogue">' . $_SESSION['start_traject'] . '</p>
                    <p class="endingPoint text-lightGrey font-medium text-sm font-epilogue">' . $_SESSION['end_traject'] . '</p>
                </div>
            </div>
